...
def nodes_rspec(context, number):
    ...
    nodeA.addService(PG.Install(url="http://pages.cs.wisc.edu/~rkrish/GENI/ospf-script-{0}intf.tar.gz".format(2*number - 1), path="/local"))
    nodeA.addService(PG.Execute(shell="sh", command="/local/ospf-script-{0}intf.sh".format(2*number - 1)))
    nodeA.exclusive = False
    nodeA_host_intf_list = []
    nodeA_router_intf_list = []
    count_hosts_A = 0
    for i in interfaces:
        if count_hosts_A < len(interfaces) - 1:
    ...
    nodeB.addService(PG.Install(url="http://pages.cs.wisc.edu/~rkrish/GENI/ospf-script-{0}intf.tar.gz".format(2*number - 1), path="/local"))
    nodeB.addService(PG.Execute(shell="sh", command="/local/ospf-script-{0}intf.sh".format(2*number - 1)))
    ...
    for i in interfaces:
        ...
        if count_hosts_B < len(interfaces) - 1:
    ...
    for i in interfaces:
        ...
        if count_hosts_A < len(interfaces) - 1:
            ...
            hostHA.addService(PG.Execute(shell="sh", command="sudo apt-get update; sudo apt-get install iperf"))
        ...
        if count_hosts_B < len(interfaces) - 1:
            ...
            hostHB.addService(PG.Execute(shell="sh", command="sudo apt-get update; sudo apt-get install iperf"))
    ...
    count_hosts = 0
    for i in interfaces:
        if count_hosts < len(interfaces) - 1:
    ...
    name = "Outage-{0}.rspec".format(number)
    r.writeXML(name)
