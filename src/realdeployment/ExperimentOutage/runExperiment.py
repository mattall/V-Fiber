...
def main(args):
    ...
    # Bring interface down randomly in one path
    toBringDown = reservoir_sampling(paths, args.teardown)
    ...
    for i in np.split(x, args.number):
        ...
        iList = np.array(indexList)
        vList = np.array(valueList)
        inds = iList.argsort()
        vals = vList[inds]
        plt.plot(sorted(iList),vals,next(linecycler),label=legend, linewidth=3)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-t", "--teardown", help="Number of links to teardown", dest="teardown", type=int)
